...
_ALIASES = {'DB': 'C#', 'EB': 'D#', 'GB': 'F#', 'AB': 'G#', 'BB': 'A#', 'AH': 'A#'}
...
def note_to_freq(name):
    ...
    body = _ALIASES.get(body.upper(), body.upper())
    if len(body) > 1 and body[1] in ('#', 'B'):
        pitch, accidental = body[0], body[1:]
        body = pitch + ('#' if accidental in ('#', 'B') else '')
    idx = _CHROMATIC.index(body)
    semitones = (idx - 9) + 12 * (octave - 4)
    return 440.0 * (2 ** (semitones / 12))


def _beat_duration(divisor, bpm=150):
    """Convert NES-style beat divisors (4=quarter, 8=eighth) to seconds."""
    return (60.0 / bpm) * (4.0 / divisor)


# Korobeiniki Type A: main theme (x2) + bridge + main theme reprise
_TYPE_A = [
    ('E5', 4), ('B4', 8), ('C5', 8), ('D5', 4), ('C5', 8), ('B4', 8),
    ('A4', 4), ('A4', 8), ('C5', 8), ('E5', 4), ('D5', 8), ('C5', 8),
    ('B4', 2 + 2 / 3), ('C5', 8), ('D5', 4), ('E5', 4),
    ('C5', 4), ('A4', 4), ('A4', 4), ('REST', 4),
    ('D5', 4), ('F5', 8), ('A5', 4), ('G5', 8), ('F5', 8),
    ('E5', 4), ('E5', 8), ('C5', 8), ('E5', 4), ('D5', 8), ('C5', 8),
    ('B4', 4), ('B4', 8), ('C5', 8), ('D5', 4), ('E5', 4),
    ('C5', 4), ('A4', 4), ('A4', 4), ('REST', 4),
]
_BRIDGE = [
    ('E5', 8), ('C5', 8), ('D5', 8), ('B4', 8), ('C5', 8), ('A4', 8),
    ('G#4', 8), ('B4', 8), ('E5', 8), ('C5', 8), ('D5', 8), ('B4', 8),
    ('C5', 8), ('E5', 4), ('A4', 8), ('G#4', 4),
]


def _build_melody():
    sequence = _TYPE_A + _BRIDGE + _TYPE_A
    return [(n, _beat_duration(d)) for n, d in sequence]
...
HELP_LINES = [
    "HELP",
    "",
    "Piece lands: +10 points.",
    "Clear a line: +100 points each.",
...
class MusicPlayer:
    def __init__(self):
        ...
        for note, duration in MELODY:
            ...
            if freq is None:
                self.sound_channels.append((None, duration))
            else:
...
class TetrisGame:
    ...
    def new_piece(self):
        ...
        self.piece_y = -len(self.current_piece)
        ...
        for r, row in enumerate(self.current_piece):
            for c, val in enumerate(row):
                ...
                if 0 <= gy < ROWS and 0 <= gx < COLS:
                    self.grid[gy][gx] = self.piece_color
    ...
    def clear_lines(self):
        ...
        if not full_rows:
            return
        self.score += len(full_rows) * LINE_SCORE
        self.grid = [row for i, row in enumerate(self.grid) if i not in full_rows]
